...
11. **Page sections only:** place the new section on a demo page (usually under `src/content/pages/examples/`) so it ships composed, not docs-only. No check guards this — it's convention.
12. **Interactive behavior?** Add a smoke test in `scripts/tests/smoke.mjs` (headless-Chrome interaction tests; see the accordion/modal/carousel tests for the pattern).
13. **Verify** (below).
...
**Runtime-built markup:** the build drops from each page the component blocks that page's markup never uses (`scripts/build/pruneCss.mjs`). It reads the built HTML and the page's JS string literals, so `classList.add("is-open")` is seen and `` el.className = `card-${variant}` `` is not. If a component's markup is only ever created at runtime from an assembled class name, add its root class to `pruneCss({ alwaysKeep: [...] })` in `astro.config.mjs`, then confirm with `npm run test:css-parity`. Components rendered by Astro need nothing.
