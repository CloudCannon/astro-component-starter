...
2. **Write `{Name}.astro`** from the matching template in [component-templates.md](component-templates.md). Destructure the standard props; compose existing building blocks over custom HTML. Components don't declare a `Props` interface — the select-option vocabulary for an enum-like prop lives in its CloudCannon `inputs.yml`. **Repeating array?** Wire the list *and* each row as editable regions before moving on — [editable-regions](../editable-regions/SKILL.md#array-repeating-items). Heading chrome alone leaves the cards unselectable in the Visual Editor.
...
11. **Interactive behavior?** Add a smoke test in `scripts/tests/smoke.mjs` (headless-Chrome interaction tests; see the accordion/modal/carousel tests for the pattern).
12. **Verify** (below).
