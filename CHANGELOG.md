...
- Code Block (`building-blocks/core-elements/code-block`): escaped code for pages and MDX, with optional language and caption labels, line numbers, long-line wrapping, and an accessible copy button enabled by default.
- Timeline Split: Timeline and Timeline Section can alternate chronological events around a centre rail, with optional icons, a dot fallback, and a single-rail mobile layout.
...
- Each page carries only the component CSS its own markup uses. Every page is built by one catch-all route, so Astro was inlining the union of the whole component library into all of them: the homepage shipped styles for pricing tables, carousels, timelines and forms it does not contain. Inline CSS is down 45% on the homepage (142KB to 79KB) and 49% across the site, and total HTML is 39.6% smaller (34.0% gzipped). On a cold Slow 4G load that is 1.26s off first contentful paint and 1.24s off largest contentful paint. Pages still inline their CSS rather than linking it, so the saving costs no extra render-blocking request. A component whose markup is only ever built at runtime from an assembled class name, or injected by a third-party script, is invisible to this — list its root class in `pruneCss({ alwaysKeep: [...] })` in `astro.config.mjs`.
