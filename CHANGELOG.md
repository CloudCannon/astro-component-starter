...
- A site-wide privacy and consent workflow, editable in CloudCannon's new Privacy & Consent and Analytics data panels. One strict opt-in policy applies to every visitor: optional analytics and external media stay off by default (and stay off when the workflow is disabled); visitors can accept analytics, reject non-essential services, manage separate analytics and external-media choices, and reopen those settings from the footer or a persistent control. Hosted video, maps, and raw embed content wait for site-wide external-media approval, provider URLs are restricted to known embed paths, and YouTube uses Privacy Enhanced Mode when loaded. The starter includes a launch-blocking privacy-policy scaffold and the current site-specific Plausible script integration.
