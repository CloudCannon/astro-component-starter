...
- Scroll Deck (`page-sections/builders/scroll-deck`): full-height cards in one track, each pinning a little lower than the last, so scrolling slides every card up over the one before it and the covered cards show as dimmed paper edges above. A rail of anchor links marks the card on top and lets a keyboard user jump between them. Cards take the same content as a carousel slide. Below 768px nothing pins, the rail is hidden and the cards stack flat.
- Scroll Stepper (`page-sections/explainers/scroll-stepper`): a walkthrough whose media column pins and stays centred while the steps scroll past it, swapping to the panel for whichever step is crossing the middle of the screen, with a progress bar or dots. Steps take the same fields as a Feature Slider slide, so content moves between the two unchanged. Below 768px the pinned column is dropped and each step shows its own image inline.
