...
- `npm run lint:roots` — no prop-driven `class`/`style`/`data-*` on a component's root element, for every component CloudCannon can make a region root (has YAML, or is named in a parent's `data-id`). The editor's re-render keeps the root and swaps its contents, so such an attribute goes stale; put it on a direct child and hoist with `:has()`. `aria-*` and the exceptions in the script's `ALLOWED` list are exempt. It also fails on a **literal** `data-editable` on a root (it collides with the one CloudCannon stamps there, and the item vanishes from the array editor) and on a classed root that spreads a rest without destructuring `class` (a spread `class` replaces the hook class outright). Part of `npm run check`.
- `npm run lint:nesting` — the `.cloudcannon/structures/*Sections` picker contexts obey the two-tier policy in `.agents/skills/create-component/cloudcannon-yaml.md`: wide slots (section content, split, stack, carousel slide, content-selector panel) take everything, narrow slots (card, grid/masonry item, bento cell, modal body, accordion panel, step) take content blocks only. It also computes reachability and fails on an exclusion another allowed wrapper routes around — the contexts form a graph, so an exclusion that isn't transitively closed restricts nothing and only makes the picker inconsistent. Part of `npm run check`.
...
- **Renaming a prop** requires updating: the `.astro` destructure, `inputs.yml`, `structure-value.yml` defaults, any `snippets.yml`, and existing content in `src/content/`. `npm run lint:cms` catches drift on both sides — YAML and content frontmatter.
- **One alias per tree**: `@component-utils/` for utils, `@navigation/`, `@core-elements/`, `@wrappers/`, `@forms/`, `@explainers/`, `@builders/`, `@page-sections/`. `@components/` is the fallback for a subtree with no alias of its own; an ESLint rule rejects `@components/utils/` and `@components/navigation/`, which resolve to the same files as the specific aliases.
- **A component's filename must be unique across the whole library.** MDX addresses components by bare filename, so a page section that shadows a building block gets a `Section` suffix (`StepsSection.astro`, `FaqSection.astro`). A blog build fails on a collision.
...
- **Interactive components** must work in the CloudCannon editor, where inline `<script>`s don't run — put setup logic in an importable module and register it in `editor-live-sync.js` (see `carousel/setup.ts`). `lint:cms` enforces this: a scripted component is either registered there or listed in the script's `EDITOR_INERT` map with the reason its editor behaviour is acceptable.
