...
`src/integrations/consent/runtime.ts` owns the `ConsentManager` singleton exposed as `window.siteConsent`. The `site-consent` record contains a policy revision, decision timestamp, and `analytics` / `externalMedia` decisions. Reads invalidate a record when its revision differs or `expiryDays` has passed; writes synchronize through `BroadcastChannel` and the storage event. Effective permission also incorporates `enabled` and Global Privacy Control. The manager publishes changes to subscribers and the `site-consent-change` window event; consumers call `isAllowed(category)` rather than reading storage themselves.

The first-visit banner exists only when analytics is configured. It offers equal-prominence **Accept All**, **Reject All**, and **Customize** actions; the dialog shows necessary preference storage as **Always on** and permits bulk or granular changes. With analytics off, external media asks only at the blocked video, map, or embed. The footer and persistent control can reopen the dialog after a choice. Disabling the workflow hides those controls and fails closed—optional services remain denied.
