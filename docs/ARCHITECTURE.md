...
- **Photo cards are JPEG, plain cards PNG**, and the URL's extension decides which the endpoint encodes (`cardFormat` in `paths.ts`), so the bytes can never disagree with what was advertised. PNG stores a photograph at roughly seven times the size; JPEG rings around the crisp text of a flat card. sharp cover-crops the source to 1200x630 before Takumi sees it, and Takumi's `measure` supplies the title's wrap points so each line gets its own plate. The plate is always black whatever `shareImageTheme` says, so a photo card draws `logoAlternateSource` rather than the theme's logo.
